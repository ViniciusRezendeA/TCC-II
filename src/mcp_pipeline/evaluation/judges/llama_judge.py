...
class LlamaJudge(OpenAICompatibleJudge):
    """Llama (llama-uncensored) rodando em llama-server local via llama.cpp.
    Geração de texto geral, usado como juiz alternativo para diversidade de avaliação.

    Lazy loading: credenciais só são validadas na primeira chamada a evaluate().
    """

    def __init__(self, judge_id: str = "llama-uncensored", model_id: str = "llama-uncensored"):
    ...
    def _ensure_initialized(self) -> None:
        ...
        bearer_token = os.getenv("LOCAL_LLM_BEARER_TOKEN")
        ...
        if not base_url or not bearer_token:
            ...
            if not bearer_token:
                missing.append("LOCAL_LLM_BEARER_TOKEN")
            raise JudgeError(f"Credenciais faltando para Llama: {', '.join(missing)}")
